...
Usage:
  ...
  python monte_carlo_ads.py --csv keywords.csv           # auto-detects keywords
  python monte_carlo_ads.py --csv data.csv --level ad_group   # force ad group level
  python monte_carlo_ads.py --csv data.csv --level keyword    # force keyword level
...
CSV formats accepted (flat):
  ...
  Keyword level  : keyword, current_spend, impressions, clicks, conversions, avg_cpc
...
CSV formats accepted (52-week weekly):
  ...
  Keyword level  : keyword, week, spend, impressions, clicks, conversions, avg_cpc
"""
...
def sample_keywords() -> List[Campaign]:
    """Built-in flat sample data (8 keywords, ~$18k/month)."""
    return [
        Campaign("buy running shoes",      3_500,  45_000,  2_800,  140, 1.25),
        Campaign("best running shoes",     2_800,  62_000,  2_200,   88, 1.27),
        Campaign("[running shoes]",        2_200,  30_000,  1_900,  114, 1.16),
        Campaign("nike running shoes",     1_800,  28_000,  1_400,   70, 1.29),
        Campaign("adidas running shoes",   1_500,  24_000,  1_200,   48, 1.25),
        Campaign("running shoes sale",     2_100,  38_000,  1_700,  102, 1.24),
        Campaign("cheap running shoes",    1_700,  52_000,  1_600,   48, 1.06),
        Campaign("running shoes for men",  2_400,  41_000,  2_100,  105, 1.14),
    ]
...
def load_campaigns_from_csv(
    ...
    df = df.drop(columns=[c for c in ("currency code", "match type", "keyword match type") if c in df.columns])
...
def _load_flat(df: pd.DataFrame, group_col: Optional[str] = None) -> Tuple[List[Campaign], str]:
    ...
    for candidate in ([group_col] if group_col else []) + ["keyword", "ad_group", "campaign", "name"]:
        if candidate and candidate in df.columns:
            name_col = candidate
            break
    else:
        raise ValueError(
            "CSV must have a 'name', 'campaign', 'ad_group', or 'keyword' column."
        )

    if name_col == "keyword":
        entity_label = "Keyword"
    elif name_col == "ad_group":
...
def _load_weekly(
    ...
    """
    if group_col == "keyword":
        entity_label = "Keyword"
    elif group_col == "ad_group":
        entity_label = "Ad Group"
    else:
        entity_label = "Campaign"
...
def parse_args():
    ...
    p.add_argument("--level",  metavar="LEVEL", default="auto",
                   choices=["auto", "campaign", "ad_group", "keyword"],
                   help="Grouping level: 'campaign', 'ad_group', 'keyword', or 'auto' (detect from CSV columns)")
...
def main():
    ...
    if args.csv:
        ...
        if args.level == "keyword":
            print("Using built-in sample keyword data (8 keywords, ~$18k/month).")
            campaigns = sample_keywords()
            entity_label = "Keyword"
        else:
